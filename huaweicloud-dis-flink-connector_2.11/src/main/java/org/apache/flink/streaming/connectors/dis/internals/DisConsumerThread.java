...
public class DisConsumerThread extends Thread {
	...
	private final AtomicReference<Tuple2<Map<TopicPartition, OffsetAndMetadata>, DisCommitCallback>> nextOffsetsToCommit;
	...
	/**
	 * Flag to indicate whether an external operation ({@link #setOffsetsToCommit(Map, DisCommitCallback)}
			...
			if (useMetrics) {
				...
				if (metrics == null) {
					...
					for (Map.Entry<MetricName, ? extends Metric> metric: metrics.entrySet()) {
						consumerMetricGroup.gauge(metric.getKey().name(), new DISMetricWrapper(metric.getValue()));

						// TODO this metric is kept for compatibility purposes; should remove in the future
						subtaskMetricGroup.gauge(metric.getKey().name(), new DISMetricWrapper(metric.getValue()));
					}
				}
			}
    ...
    Consumer<byte[], byte[]> getConsumer(Properties kafkaProperties) {
		return new DISKafkaConsumer<>(kafkaProperties);
	}
	...
	private class CommitCallback implements OffsetCommitCallback
    {
		...
		private final DisCommitCallback internalCommitCallback;

		CommitCallback(DisCommitCallback internalCommitCallback) {
