...
import org.apache.flink.streaming.connectors.dis.internals.metrics.DISKafkaConsumerMetricConstants;
...
public abstract class AbstractFetcher<T, KPH> {
	...
	protected AbstractFetcher(
		...
		for (DisStreamPartitionState partitionState : subscribedPartitionStates) {
	 ...
	 * {@link DisStreamPartitionStateSentinel#EARLIEST_OFFSET} as the starting offset.
	 ...
	 */
	public final void commitInternalOffsetsToDis(
		...
		// replaced sentinels
		doCommitInternalOffsetsToDis(filterOutSentinels(offsets), commitCallback);
	}

	protected abstract void doCommitInternalOffsetsToDis(
			Map<DisStreamPartition, Long> offsets,
			@Nonnull DisCommitCallback commitCallback) throws Exception;

	private Map<DisStreamPartition, Long> filterOutSentinels(Map<DisStreamPartition, Long> offsets) {
		return offsets.entrySet()
			.stream()
			.filter(entry -> !DisStreamPartitionStateSentinel.isSentinel(entry.getValue()))
	 ...
	 */
	protected abstract KPH createKafkaPartitionHandle(DisStreamPartition partition);
	 ...
	 */
	public HashMap<DisStreamPartition, Long> snapshotCurrentState() {
	 ...
	 * Per-partition metric groups can be scoped by user variables {@link DISKafkaConsumerMetricConstants#OFFSETS_BY_TOPIC_METRICS_GROUP}
	 * and {@link DISKafkaConsumerMetricConstants#OFFSETS_BY_PARTITION_METRICS_GROUP}.
	 *
