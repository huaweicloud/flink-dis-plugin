...
import static org.apache.flink.streaming.connectors.dis.internals.metrics.DISKafkaConsumerMetricConstants.*;
import static org.apache.flink.util.Preconditions.checkArgument;
import static org.apache.flink.util.Preconditions.checkNotNull;

/**
 * Base class of all Flink DIS Consumer data sources.
 *
 * @param <T> The type of records produced by this data source
 */
@Internal
public abstract class FlinkDisConsumerBase<T> extends RichParallelSourceFunction<T> implements
    CheckpointListener,
    ...
    CheckpointedFunction
{
	...
	protected static final Logger LOG = LoggerFactory.getLogger(FlinkDisConsumerBase.class);
	...
	/** The schema to convert between DIS's byte messages, and Flink's objects. */
	protected final DisDeserializationSchema<T> deserializer;

	/** The set of topic partitions that the source will read, with their initial offsets to start reading from. */
	private Map<DisStreamPartition, Long> subscribedPartitionsToStartOffsets;
	 ...
	 * The value of this can only be determined in {@link FlinkDisConsumerBase#open(Configuration)} since it depends
	...
	private Map<DisStreamPartition, Long> specificStartupOffsets;
	...
	/** The fetcher implements the connections to the DIS. */
	private transient volatile AbstractFetcher<T, ?> disFetcher;
	 ...
	 */
	private transient volatile TreeMap<DisStreamPartition, Long> restoredState;

	/** Accessor for state in the operator state backend. */
	private transient ListState<Tuple2<DisStreamPartition, Long>> unionOffsetStates;
	...
	/** Counter for successful DIS offset commits. */
	private transient Counter successfulCommits;

	/** Counter for failed DIS offset commits. */
	private transient Counter failedCommits;

	/** Callback interface that will be invoked upon async DIS commit completion.
	 ...
	 *  supported DIS connectors guarantee no more than 1 concurrent async pending offset
	 *  commit.
	 */
	private transient DisCommitCallback offsetCommitCallback;
	 ...
	 */
	public FlinkDisConsumerBase(
			List<String> topics,
			Pattern topicPattern,
			DisDeserializationSchema<T> deserializer,
			long discoveryIntervalMillis,
			boolean useMetrics) {
		this.topicsDescriptor = new DisStreamsDescriptor(topics, topicPattern);
	 ...
	 */
	public FlinkDisConsumerBase<T> assignTimestampsAndWatermarks(
	 ...
	 * The watermark extractor will run per DIS partition, watermarks will be merged across partitions
	 * in the same way as in the Flink runtime, when streams are merged.
	 *
	 * <p>When a subtask of a FlinkDISConsumer source reads multiple DIS partitions,
	 ...
	 * per DIS partition, they will not be strictly ascending in the resulting Flink DataStream, if the
	 * parallel source subtask reads more than one partition.
	 *
	 * <p>Running timestamp extractors / watermark generators directly inside the DIS source, per DIS
	 ...
	 */
	@Deprecated
	public FlinkDisConsumerBase<T> assignTimestampsAndWatermarks(AssignerWithPunctuatedWatermarks<T> assigner) {
	 ...
	 */
	@Deprecated
	public FlinkDisConsumerBase<T> assignTimestampsAndWatermarks(AssignerWithPeriodicWatermarks<T> assigner) {
		checkNotNull(assigner);
	...
	/**
	 * Specifies whether or not the consumer should commit offsets back to DIS on checkpoints.
	 *
	 ...
	 * @return The consumer object, to allow function chaining.
	 */
	public FlinkDisConsumerBase<T> setCommitOffsetsOnCheckpoints(boolean commitOnCheckpoints) {
	 ...
	 * This lets the consumer ignore any committed group offsets in DIS.
	 *
	 ...
	 * @return The consumer object, to allow function chaining.
	 */
	public FlinkDisConsumerBase<T> setStartFromEarliest() {
	 ...
	 * @return The consumer object, to allow function chaining.
	 */
	public FlinkDisConsumerBase<T> setStartFromLatest() {
	 ...
	 * to the specific timestamp from DIS. If there's no such offset, the consumer will use the
	 * latest offset to read data from DIS.
	 *
	 * <p>This method does not affect where partitions are read from when the consumer is restored
	 * from a checkpoint or savepoint. When the consumer is restored from a checkpoint or
	 * savepoint, only the offsets in the restored state will be used.
	 ...
	 * in DIS. The "group.id" property must be set in the configuration properties.
	 * If no offset can be found for a partition, the behaviour in "auto.offset.reset"
	 ...
	 * @return The consumer object, to allow function chaining.
	 */
	public FlinkDisConsumerBase<T> setStartFromGroupOffsets() {
	 ...
	 * This lets the consumer ignore any committed group offsets in DIS.
	 *
	 * <p>If the provided map of offsets contains entries whose {@link DisStreamPartition} is not subscribed by the
	 ...
	 * {@link FlinkDisConsumerBase#setStartFromGroupOffsets()}) for that particular partition.
	 *
	 ...
	 * @return The consumer object, to allow function chaining.
	 */
	public FlinkDisConsumerBase<T> setStartFromSpecificOffsets(Map<DisStreamPartition, Long> specificStartupOffsets) {
		...
		if (restoredState != null) {
			for (DisStreamPartition partition : allPartitions) {
				if (!restoredState.containsKey(partition)) {
					restoredState.put(partition, DisStreamPartitionStateSentinel.EARLIEST_OFFSET);
				}
			}

			for (Map.Entry<DisStreamPartition, Long> restoredStateEntry : restoredState.entrySet()) {
				// seed the partition discoverer with the union state while filtering out
				// restored partitions that should not be subscribed by this subtask
				if (DisStreamPartitionAssigner.assign(
			...
			switch (startupMode) {
				case SPECIFIC_OFFSETS:
					...
					for (DisStreamPartition seedPartition : allPartitions) {
						...
						if (specificOffset != null) {
							...
							subscribedPartitionsToStartOffsets.put(seedPartition, DisStreamPartitionStateSentinel.GROUP_OFFSET);
						}
					}
				...
				case TIMESTAMP:
					...
					for (Map.Entry<DisStreamPartition, Long> partitionToOffset
						...
						subscribedPartitionsToStartOffsets.put(
							...
							(partitionToOffset.getValue() == null)
									...
									? DisStreamPartitionStateSentinel.LATEST_OFFSET
									// since the specified offsets represent the next record to read, we subtract
									// it by one so that the initial state of the consumer will be correct
			...
			if (!subscribedPartitionsToStartOffsets.isEmpty()) {
				switch (startupMode) {
					...
					case SPECIFIC_OFFSETS:
						...
						List<DisStreamPartition> partitionsDefaultedToGroupOffsets = new ArrayList<>(subscribedPartitionsToStartOffsets.size());
						for (Map.Entry<DisStreamPartition, Long> subscribedPartition : subscribedPartitionsToStartOffsets.entrySet()) {
							if (subscribedPartition.getValue() == DisStreamPartitionStateSentinel.GROUP_OFFSET) {
						...
						if (partitionsDefaultedToGroupOffsets.size() > 0) {
							LOG.warn("Consumer subtask {} cannot find offsets for the following {} partitions in the specified startup offsets: {}" +
									"; their startup offsets will be defaulted to their committed group offsets in DIS.",
								getRuntimeContext().getIndexOfThisSubtask(),
					...
					default:
					case GROUP_OFFSETS:
						LOG.info("Consumer subtask {} will start reading the following {} partitions from the committed group offsets in DIS: {}",
							getRuntimeContext().getIndexOfThisSubtask(),
							subscribedPartitionsToStartOffsets.size(),
							subscribedPartitionsToStartOffsets.keySet());
				}
	...
	public void run(SourceContext<T> sourceContext) throws Exception {
		...
		this.offsetCommitCallback = new DisCommitCallback() {
			@Override
			...
			public void onException(Throwable cause) {
				LOG.warn("Async DIS commit failed.", cause);
				failedCommits.inc();
			}
		};
		...
		//     DIS through the fetcher, if configured to do so)
		this.disFetcher = createFetcher(
				sourceContext,
				subscribedPartitionsToStartOffsets,
				watermarkStrategy,
				(StreamingRuntimeContext) getRuntimeContext(),
				offsetCommitMode,
				getRuntimeContext().getMetricGroup().addGroup(DIS_CONSUMER_METRICS_GROUP),
				useMetrics);
		...
		if (discoveryIntervalMillis != PARTITION_DISCOVERY_DISABLED) {
			...
			}, "DIS Partition Discovery for " + getRuntimeContext().getTaskNameWithSubtasks());

			discoveryLoopThread.start();
			disFetcher.runFetchLoop();
	...
	public final void initializeState(FunctionInitializationContext context) throws Exception {
		...
		if (context.isRestored()) {
			restoredState = new TreeMap<>(new DisStreamPartition.Comparator());

			// populate actual holder for restored state
			for (Tuple2<DisStreamPartition, Long> disOffset : unionOffsetStates.get()) {
				restoredState.put(disOffset.f0, disOffset.f1);
			}

			LOG.info("Setting restore state in the FlinkDisConsumer: {}", restoredState);
		} else {
			LOG.info("No restore state for FlinkDisConsumer.");
		}
	}
	...
	public final void snapshotState(FunctionSnapshotContext context) throws Exception {
		if (!running) {
			...
			final AbstractFetcher<?, ?> fetcher = this.disFetcher;
			if (fetcher == null) {
				...
				HashMap<DisStreamPartition, Long> currentOffsets = fetcher.snapshotCurrentState();
				...
				for (Map.Entry<DisStreamPartition, Long> disTopicPartitionLongEntry : currentOffsets.entrySet()) {
					unionOffsetStates.add(
							Tuple2.of(disTopicPartitionLongEntry.getKey(), disTopicPartitionLongEntry.getValue()));
				}
			}
	...
	public final void notifyCheckpointComplete(long checkpointId) throws Exception {
		...
		if (offsetCommitMode == OffsetCommitMode.ON_CHECKPOINTS) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Committing offsets to DIS for checkpoint " + checkpointId);
			}
			...
			try {
				...
				Map<DisStreamPartition, Long> offsets =
					(Map<DisStreamPartition, Long>) pendingOffsetsToCommit.remove(posInMap);
				...
				fetcher.commitInternalOffsetsToDis(offsets, offsetCommitCallback);
			} catch (Exception e) {
				if (running) {
					throw e;
				}
	...
	// ------------------------------------------------------------------------
	//  DIS Kafka Consumer specific methods
	// ------------------------------------------------------------------------

	/**
	 * Creates the fetcher that connect to the DIS, pulls data, deserialized the
	...
	protected abstract AbstractFetcher<T, ?> createFetcher(
			...
			Map<DisStreamPartition, Long> subscribedPartitionsToStartOffsets,
			SerializedValue<WatermarkStrategy<T>> watermarkStrategy,
			StreamingRuntimeContext runtimeContext,
			OffsetCommitMode offsetCommitMode,
			MetricGroup disMetricGroup,
	...
	protected abstract Map<DisStreamPartition, Long> fetchOffsetsWithTimestamp(
	...
	@VisibleForTesting
	Map<DisStreamPartition, Long> getSubscribedPartitionsToStartOffsets() {
		return subscribedPartitionsToStartOffsets;
	}

	@VisibleForTesting
	TreeMap<DisStreamPartition, Long> getRestoredState() {
	...
	/**
	 * Creates state serializer for DIS Stream partition to offset tuple.
	 * Using of the explicit state serializer with KryoSerializer is needed because otherwise
	 * users cannot use 'disableGenericTypes' properties with DISConsumer.
	 */
	@VisibleForTesting
	static TupleSerializer<Tuple2<DisStreamPartition, Long>> createStateSerializer(ExecutionConfig executionConfig) {
		...
		TypeSerializer<?>[] fieldSerializers = new TypeSerializer<?>[]{
			new KryoSerializer<>(DisStreamPartition.class, executionConfig),
			LongSerializer.INSTANCE
		};
		@SuppressWarnings("unchecked")
		Class<Tuple2<DisStreamPartition, Long>> tupleClass = (Class<Tuple2<DisStreamPartition, Long>>) (Class<?>) Tuple2.class;
