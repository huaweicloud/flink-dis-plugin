...
/**
 * A special version of the per-dis-partition-state that additionally holds a {@link
 ...
 * @param <KPH> The type of the DIS partition descriptor, which varies across DIS versions.
 */
@Internal
public final class DisTopicPartitionStateWithWatermarkGenerator<T, KPH> extends DisStreamPartitionState<T, KPH> {
	...
	public DisTopicPartitionStateWithWatermarkGenerator(
			DisStreamPartition partition, KPH disPartitionHandle,
			TimestampAssigner<T> timestampAssigner,
			WatermarkGenerator<T> watermarkGenerator,
			WatermarkOutput immediateOutput,
			WatermarkOutput deferredOutput) {
		super(partition, disPartitionHandle);
	...
	@Override
	public long extractTimestamp(T record, long disEventTimestamp) {
		return timestampAssigner.extractTimestamp(record, disEventTimestamp);
	}
	...
	public String toString() {
		return "DisTopicPartitionStateWithPeriodicWatermarks: partition=" + getKafkaTopicPartition()
				+ ", offset=" + getOffset();
	}
}
